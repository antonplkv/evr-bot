...
def start_message(message):
    ...
    tBot.print_keyboard_picture(message.chat.id,'https://evrasia.in.ua/images/evrasiabot/start.png',message_keyboard)
...
def rest_chosen_number(call):
    ...
    if get_phone(call.message.chat.id) == -1:
        ...
        message_text = 'Будь-ласка, введіть номер телефону. \nЗараз: ' + str(get_phone(call.message.chat.id))
...
def chooserest(call):
    ...
    # </editor-fold>
    inline_keyboard = get_inline_keyboard(keyboard_tuple(in_buttons, 1))  # inline-клавиатура с ресторанами
    # </editor-fold>
...
def tempPressedPay(message):
    ...
    tBot.print_keyboard_message(message.chat.id, 'Будь-ласка, оберіть адресу:', rest_keyboard(restaurant_list))
...
def callback_inline(call):
    ...
    if len(cat_list) > 0:
        ...
        # TODO отвязать от жестких строк. Предусмотреть мультиязычность
        tBot.update_message_text(call.message.chat.id, call.message.message_id, 'Оберіть розділа', inline_keyboard)
    # </editor-fold>
    # Если нет подкатегорий и есть только товары
        ...
        for i in range(0, meals):
            ...
            tBot.sendPhoto(call.message.chat.id,cat_list[i][5],message_str,inline_keyboard)
            # </editor-fold>
    ...
    def fivemore(m):
        ...
        if not pre_last and rest_meals < 5:
            ...
            pre_last = True
            print(step,rest_meals,pre_last)
        ...
        for i in range(next_meals, step):
            ...
            message_str = '<b>{0}</b>\n\nВага: {1} г \n<b>Ціна: {2} грн</b>\n\n {3}'.format(cat_list[i][3],
                                                                                            ...
                                                                                            cat_list[i][4])
            types.PhotoSize(cat_list[i][5], width=400, height=400)
            tBot.sendPhoto(id22, cat_list[i][5], message_str, inline_keyboard)
        tBot.print_keyboard_message(id22, 'Меню', message_keyboard)
...
def orders_service(message):
    ...
    if saved_orders != [[-1]]:
        ...
        for i in range(0, len(saved_orders)):
            in_buttons.append(types.InlineKeyboardButton(text=str(get_order_date(message.chat.id,i)), callback_data='so_{0}'.format(str(i+1))))
        # </editor-fold>
...
def settings_phone(message):
    ...
    if formattedPhone is False:
        message_keyboard = types.ReplyKeyboardMarkup(one_time_keyboard=True, resize_keyboard=True)
        tBot.print_keyboard_message(message.chat.id, 'Невірно введений номер', message_keyboard)
        tBot.print_keyboard_message(message.chat.id, "Введіть коректний номер", message_keyboard='')
    else:
        ...
        tBot.print_keyboard_message(message.chat.id, textPhone, message_keyboard)
        set_state(message.chat.id, 1)
...
def handle_message(message):
    if get_state(message.chat.id) == 5:
        ...
        db_con.close()
        # </editor-fold>
        #message_text = 'Заказ оформлено \nВаш заказ на суму {0} грн відправлено \nЗ вами зв\'яжеться оператор для '\
        #'уточнення деталей замовлення\nДякуємо за те, що Ви з нами!\nЗамовлення оброблюються оператором з 10:00' \
        #' до 20:00.!'.format(str(int(get_full_price(message.chat.id))))
	message_text = 'Замовлення оформлено \nВаше замовлення на суму {0} грн відправлено \nЗ Вами зв\'яжеться оператор для '\
        'уточнення деталей замовлення\nДякуємо за те, що Ви з нами!\nЗамовлення опрацьовуються оператором з 10:00' \
        ' до 00:00.!'.format(str(int(get_full_price(message.chat.id))))
        ...
        if(message.text == '✅Вірно'):
            ...
            print(user_phone)
            correct_number = check_phone(user_phone)#проверка корректности номера
        ...
        if correct_number:
            ...
            message_text +='\nАдреса ресторану {0}'.format(restaurants.restaurants[int(get_rest(message.chat.id))])
            cursor.close()
            db_con.close()
            # </editor-fold>
            ...
            data = {'user': user, 'restaurant_id': str(get_rest(message.chat.id)), 'order': order}
            json_data = json.dumps(data, separators=(',', ': '))
            print(json_data)
            encode_data = base64.b64encode(json_data.encode())
            r = requests.post('http://10.20.100.200/telegram/import', data={'code': encode_data})
            print(r)
            # </editor-fold>
            saved_orders = get_saved_orders(message.chat.id)
            if saved_orders == [[-1]] or len(saved_orders) < 4:
