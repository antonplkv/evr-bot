...
def messageNotExpected(message):
    tBot.print_without_keyboard(message.chat.id,"Будьте ласкаві, оберіть налаштування ")
...
def handle_message(message):
    if get_state(message.chat.id) == 5:
        ...
        if correct_number:
            ...
            db_con = connection.MySQLConnection(user=config.db_user, password=config.db_password, host=config.db_host,
                                                database=config.db_name)
            query = 'SELECT online_code FROM restaurants where id = {}'.format(restaurant_id)
            cursor = db_con.cursor()
            cursor.execute(query)
            for (REST_ID,) in cursor:
...
def unexpectedMessage(message):
    tBot.print_without_keyboard(message.chat.id,"Будь ласка, оберіть ресторан")

@tBot.bot.message_handler(func = WrongMessageAtHome)
def unexpectedMessage(message):
    tBot.print_without_keyboard(message.chat.id,'Будь лаcка, оберіть пункт меню')
...
def unexpectedMessage(message):
    tBot.print_without_keyboard(message.chat.id,'Будьте ласка, оберіть налатування')

@tBot.bot.message_handler(func = WrongMessageAtMyOrders)
def messageNotExpected(message):
    tBot.print_without_keyboard(message.chat.id,'Будь ласка, оберіть замовлення')

@tBot.bot.message_handler(func = WrongMessageAtPhonEntered)
def messageNotExpected(message):
   tBot.print_without_keyboard(message.chat.id,'Будь ласка, оберіть пункт меню')
#</editor-fold>
